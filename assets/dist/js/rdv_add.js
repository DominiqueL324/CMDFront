...
function getClient(cas=0,val_=1){
        ...
        $.ajax({
            ...
            success: function(response){
                ...
                $('#client').append(
                        ...
                        <select onchange='getPasseur()' class='form-select form-control form-select-lg' id='client_val'> "+ content + "</select>"
                )
                if(cas != 0){
                    $('#client').empty()
                    $('#client').append(
    ...
    if($.cookie("group") == "Client pro" || $.cookie("group")=="client particulier"){
        ...
        $('#client').empty()
        $('#client').append("<label for='exampleInputEmail1'>Client</label>\
        <input readonly=''  class='form-select form-control ' value= \'"+ content + "\'/>")
        getPasseur(cas=0)
    }
}

function getPasseur(cas=0){
    var id_client =0
    if($.cookie('group')=="Client pro" || $.cookie('group')=="Client particulier"){
        id_client =  $.cookie('id_user_logged')
    }
    if($.cookie('group')=="Salarie"){
        content = $.cookie('name')+" "+$.cookie('first_name')
        $('#passeur').empty()
        $('#passeur').append("<label for='exampleInputEmail1'>Passeur</label>\
        <input readonly=''  class='form-select form-control ' value= \'"+ content + "\'/>")
    }
    $.ajax({
        type: 'GET',
        ...
        success: function(response){
            ...
            $('#passeur').append(
                    ...
                    <select  class='form-select form-control form-select-lg' id='passeur_val'> "+ content + "</select>"
            )
            if(cas != 0){
                $('#passeur').empty()
                $('#passeur').append(
...
function getAgent(cas=1,val_=0,){
        ...
        $.ajax({
            ...
            success: function(response){
                ...
                if(cas!=1){
                    ...
                    $('#agent').append(
                        " <label for='exampleInputEmail1'>Agent de secteur</label>\
                        <select class='form-select form-control form-select-lg' id='agent_val'> "+ content + "</select>"
                    )
                }
                if(val_ !=0){
                    $('#agent').empty()
                    $('#agent').append(
                        " <label for='exampleInputEmail1'>Agent de secteur</label>\
                        <select disabled=''  class='form-select form-control form-select-lg' id='agent_val'> "+ content + "</select>"
                    )
    ...
    if($.cookie('group')=="Agent secteur" || $.cookie('group')=="Agent constat" || $.cookie('group')=="Audit planneur"){
        content = $.cookie('name')+" "+$.cookie('first_name')
        $('#agent').empty()
        $('#agent').append("<label for='exampleInputEmail1'>Agent de secteur</label>\
            <input readonly=''  class='form-select form-control ' value= "+ content + "/>")
    }
    if($.cookie('group')=="Client pro" || $.cookie('group')=="Client particulier"){
    ...
    $.ajax({
        ...
        success: function(response){
            ...
            $('#intervention').append(
                    ...
                    <select  class='form-select form-control form-select-lg' id='intervention_val'> "+ content + "</select>"
            )
            if(val_ != 0){
    ...
    $.ajax({
        ...
        success: function(response){
            ...
            $('#propriete').append(
                    ...
                    <select  class='form-select form-control form-select-lg' id='propriete_val'> "+ content + "</select>"
            )
            if(val1 != 0){
...
function addRdv(){
    ...
    if($.cookie('group')=="Salarie"){
        ...
        data['client'] = $.cookie("id_client_sal")
        data['agent'] = $.cookie("id_user_agent")
    }
...
function getRdvToEdit(){
    ...
    $.ajax({
        ...
        success: function(response){
            ...
            var formattedDate = new Date(response[0]["date"]).toISOString().split('T')[0];
