...
$('#type_user').on('change',function(){
    if($('#type_user').val()=="3" || $('#type_user').val()=="5" || $('#type_user').val()=="6" ){
    ...
    $.ajax({
        ...
        success: function(response){
            ...
            response.forEach(elt => {
                 content = content + "<option value = " + elt['id'] + ">" + elt['user']['nom'] + "  " + elt['user']['prenom'] + "</option>"
            });
            if(cas!=1){
                $('#as').empty()
                $('#as').append(
                    " <label for='exampleInputEmail1'>Agent de secteur</label>\
                    <select disabled=''  class='form-select form-control form-select-lg' id='as_val'> "+ content + "</select>"
                )
            }else{
                $('#as').empty()
                $('#as').append(
                    " <label for='exampleInputEmail1'>Agent de secteur</label>\
                    <select class='form-select form-control form-select-lg' id='as_val'> "+ content + "</select>"
                )
            }
            if(val !=1){
    ...
    $.ajax({
        ...
        success: function(response){
            ...
            $('#client').append(
                    ...
                    <select onchange='getboyCl()' class='form-select form-control form-select-lg' id='client_val'> "+ content + "</select>"
            )
            if(cas == 1){
                $("#client_val").val(val)
            }
            if(val_!=1){
                ...
                $('#client').append(
                    " <label for='exampleInputEmail1'>Client</label>\
                    <select disabled='' class='form-select form-control form-select-lg' id='client_val'> "+ content + "</select>"
            )
            }
...
function addUser(){
    ...
    if(!$('#login').val() || !$('#nom').val() || !$('#prenom').val() || !$('#mdp').val() || !$('#email').val() || !$('#adresse').val() || !$('#telephone').val()){
    ...
    $.ajax({
        ...
        success: function(response){
            ...
            if(Edition==1){
                ...
                //info concession
                $('#secteur').val(response[0]["info_concession"]["agent_rattache"]["secteur"]+" "+response[0]["info_concession"]["agent_rattache"]["secteur_secondaire"])
                $('#nom_concessionanire').val(response[0]["info_concession"]["nom_concessionnaire"])
                $('#suive_technique').val(response[0]["info_concession"]["suivie_technique_client"])
                $('#origine_client').val(response[0]["info_concession"]["origine_client"])
                $('#numero_proposition').val(response[0]["info_concession"]["numero_proposition_prestation"])
                ...
                if(response[0]["ref_comptable"] != null){
                    ...
                    $('#email_envoi_facture').val(response[0]["ref_comptable"]["email_envoi_facture"])
                    $('#telephone_comptable').val(response[0]["ref_comptable"]["telephone"])
                }
                
                //info gestion
                if(response[0]["ref_service_gestion"] != null){
                    $('#nom_gestionnaire').val(response[0]["ref_service_gestion"]["nom_complet"])
                    $('#mobile_service_gestion').val(response[0]["ref_service_gestion"]["mobile"])
                    $('#email_gestionnaire').val(response[0]["ref_service_gestion"]["email"])
                    $('#telephone_service_gestion').val(response[0]["ref_service_gestion"]["telephone"])
                }
                getAs(2)
                $('#as').css('display','inline')
                ...
                $('#secteur').val(response[0]["info_concession"]["agent_rattache"]["secteur"]+" "+response[0]["info_concession"]["agent_rattache"]["secteur_secondaire"])
                $('#nom_concessionanire').val(response[0]["info_concession"]["nom_concessionnaire"])
                $('#suivie_technique').val(response[0]["info_concession"]["suivie_technique_client"])
                $('#origine_client').val(response[0]["info_concession"]["origine_client"])
                $('#numero_proposition').val(response[0]["info_concession"]["numero_proposition_prestation"])
                $('#asclient').val(response[0]["info_concession"]["as_client"])
                //$('#statut').val(response[0]["statut_client"])
...
function editUser(cas,prof=0){
    ...
    if(cas==1){
        ...
        if($('#role').val() == "Agent constat" || $('#role').val() == "Audit planneur"){
            if($('#as_val').val() == "0"){
                $('#as_error').css("display","inline")
                return
            }
